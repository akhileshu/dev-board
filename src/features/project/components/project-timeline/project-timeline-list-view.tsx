...
function ProjectTimelineMinimalInfo({ projectTimeline }: ProjectTimelineMinimalInfoProps) {
  return (
    <Link key={ ProjectTimeline.id} href={`/projectTimelines/${ ProjectTimeline.id}`}>
...
type ProjectTimelineListViewProps = {
  className?: string;
  projectTimelinesResult: Awaited<ReturnType<typeof projectTimelineActions.getAll>>;
};
